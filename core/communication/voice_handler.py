...
import time
import json
from typing import Dict, Any, List, Optional, Callable, Coroutine, Tuple # Ensured all are imported
...
class SalesAgent:
    ...
    def __init__(
        ...
        if self.voice_handler:
            ...
            logger.error(f"SalesAgent {self.agent_id}: VoiceHandler is None at initialization. Callbacks cannot be set. This will likely lead to issues.")


        logger.info(f"SalesAgent {self.agent_id} initialized for {self.target_phone_number} in niche '{self.target_niche}'.")
    ...
    async def start_sales_call(self, call_sid: str, stream_sid: str):
        ...
        try:
            if not self.voice_handler:
                raise ConnectionError("VoiceHandler not initialized for SalesAgent.")
    ...
    async def _manage_call_lifecycle(self):
        ...
        try:
            ...
            await asyncio.wait_for(self.call_end_event.wait(), timeout=float(self.max_call_duration))
            ...
            if self._final_status_reason != "Cleanup Complete":
    ...
    async def _verify_pending_action(self, user_response: str):
        if not self._pending_action_verification:
            # If no action was pending, but we got a transcript, initiate next turn.
            # This can happen if LLM didn't propose an action requiring verification.
            await self._initiate_agent_turn()
            return
            
        if not self.llm_client: 
            logger.warning(f"[{self.agent_id}] LLM client unavailable for action verification. Clearing pending action and initiating next turn.")
    ...
    async def _determine_next_strategic_goal(self) -> Tuple[str, str]:
        ...
        messages = [{"role": "user", "content": prompt}]
        response_str = None # Initialize for robust error handling
        try:
            ...
            if response_str and '{' in response_str and '}' in response_str:
                ...
                logger.warning(f"[{self.agent_id}] LLM failed to provide valid JSON for strategy goal. Raw: {str(response_str)[:200]}. Defaulting.")
        except Exception as e: 
            logger.error(f"[{self.agent_id}] Error determining strategy goal: {e}. Defaulting. Raw response (if any): {str(response_str)[:200]}", exc_info=True)
    ...
    async def _initiate_agent_turn(self):
        if not self.is_call_active or not self.call_sid: 
            return
        if not self.llm_client: 
            logger.error(f"[{self.agent_id}] Cannot initiate agent turn: LLMClient not available.")
            # Potentially send a pre-recorded "technical difficulties" message if possible, or hang up.
            await self._handle_fatal_error("LLMClient unavailable during agent turn.")
            return
        try:
            if not self._pending_action_verification:
            ...
            if self.is_call_active and self.voice_handler:
                ...
                logger.error(f"[{self.agent_id}] VoiceHandler not available for TTS streaming.")
                await self._handle_fatal_error("VoiceHandler unavailable for TTS.")
            ...
            if self.is_call_active:
                try:
                    ...
                    if self.voice_handler:
                        ...
                        logger.error(f"[{self.agent_id}] VoiceHandler not available for fallback TTS.")
    ...
    async def _stream_tts(self, text_to_speak: str):
        if not self.is_call_active or not self.call_sid or not self.stream_sid or not self.voice_handler: 
            if not self.voice_handler: logger.error(f"[{self.agent_id}] TTS stream: VoiceHandler not available.")
            return
    ...
    async def _generate_call_summary_with_llm(self) -> Dict[str, Any]:
        ...
        messages = [{"role": "user", "content": prompt}]
        response_str = "Error: LLM summary generation failed before API call." 
        try:
            ...
            logger.error(f"[{self.agent_id}] Error generating LLM call summary: {e}. Raw: '{str(response_str)[:200]}'", exc_info=True)
    ...
    async def _cleanup_call(self, final_status: str, error_message: Optional[str]):
        if self._final_status_reason == "Cleanup Complete": 
            logger.warning(f"[{self.agent_id}] Cleanup called again for {self.call_sid}, but already complete.")
            return

        current_call_sid = self.call_sid or f"UnknownSID_Cleanup_{self.agent_id}"
        ...
        if self.call_lifecycle_task and not self.call_lifecycle_task.done():
            if asyncio.current_task() is not self.call_lifecycle_task:
                ...
                except asyncio.CancelledError: logger.info(f"[{self.agent_id}][{current_call_sid}] Call lifecycle task cancelled.")
                except Exception as e_clt: logger.error(f"[{self.agent_id}][{current_call_sid}] Error awaiting cancelled lifecycle task: {e_clt}")
        ...
        if self.telephony_wrapper and current_call_sid != f"UnknownSID_Cleanup_{self.agent_id}":
        ...
        if "Error" in final_status and "Error" not in logged_outcome : 
        ...
        if "summary_status" in llm_summary and (llm_summary["summary_status"].startswith("llm_") or llm_summary["summary_status"].startswith("Error_")):
        ...
        if self.crm_wrapper:
            log_success = await self.crm_wrapper.log_call_outcome(
                ...
                llm_call_summary_json=llm_summary if "summary_status" not in llm_summary or llm_summary["summary_status"] == "success" else None,
            ...
            if contact_id_for_log and logged_outcome:
                new_status = f"PostCall_{logged_outcome.replace(' ', '_')}"
